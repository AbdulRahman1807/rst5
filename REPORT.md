...
| Kafka/Neo4j volumes | Explicit named volumes (`kafka_data`, `neo4j_data`, `neo4j_logs`) | Relying on the official images' implicit anonymous volumes (no `volumes:` entry at all) | Verified live: anonymous volumes for both images were observed surviving `docker compose down -v` in this environment — Kafka topic data and Neo4j's graph persisted across a "clean" reset, so a second upload of the same file showed inflated counts (accumulated across every prior test run, not the fresh single-file state `down -v` is supposed to guarantee). This directly threatens the judges' own reproducibility check (handout: `down -v` then `up` on the test CSV, counts must match). Named volumes are removed reliably by `down -v`; confirmed by a full down/up/re-upload cycle showing the correct row count with nothing left over |
